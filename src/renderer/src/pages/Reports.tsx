...
export default function ReportsPage() {
  ...
  const { data: branches = [], error: branchesError } = useSWR<Branch[]>("branches", fetchBranches, {
    revalidateOnFocus: false,
  });
  if (branchesError) showToast(branchesError.message, "error");

  // ---------- SWR: Report ----------
  const fetchReport = async () => {
    const res = await window.api.getDetailedReport({
  ...
  const { data: reportResult, error: reportError } = useSWR(
    ["report", selectedBranch, startDate, endDate, page, type],
    fetchReport,
    { revalidateOnFocus: false }
  );
  if (reportError) showToast(reportError.message, "error");

  const reportData: ReportMovement[] = reportResult?.data || [];
  const total: number = reportResult?.total || 0;
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-700">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Movimentos Detalhados</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-slate-700 border-b border-gray-200 dark:border-slate-600">
              <tr>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Data</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Filial Origem</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Filial Destino</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Código Produto</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Produto</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Qtd</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Entrada/Saída</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Nf de entrada</th>
                <th className="p-4 text-left text-sm font-semibold text-gray-900 dark:text-white">Observações</th>
              </tr>
            </thead>
            <tbody>
              {reportData.length === 0 ? (
                ...
                reportData.map((item, idx) => (
                  <tr key={idx} className="border-b border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className={`p-4 text-sm font-semibold ${
                      item.type === "saida" ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"
                    }`}>
          ...
          {total > pageSize && (
            <div className="flex items-center justify-center gap-3 mt-6 mb-6">
              ...
              <div className="px-4 py-2 rounded-sm bg-[#2c5396] text-white font-medium shadow">{page} / {Math.ceil(total / pageSize)}</div>
              <button
